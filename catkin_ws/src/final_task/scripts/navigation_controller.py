...
class NavigationController:
    ...
    def __init__(self):
        ...
        # 获取停车点坐标
        self.parking_goal = rospy.get_param('~parking', [-3.213, 3.357, 3.138])
        rospy.loginfo(f"停车点坐标: {self.parking_goal}")
        self.parking_coord_tolerance = 0.5 # 50cm tolerance for identifying the parking goal
    ...
    def is_parking_goal(self, x, y, yaw):
        """检查目标是否为停车点"""
        if len(self.parking_goal) != 3:
            return False
        
        # Check distance
        dist_sq = (x - self.parking_goal[0])**2 + (y - self.parking_goal[1])**2
        rospy.loginfo(f"Distance squared to parking goal: {dist_sq}, tolerance squared: {self.parking_coord_tolerance**2}")
        # Use a slightly larger tolerance for identification than for PID success
        return dist_sq < self.parking_coord_tolerance**2
    
    def navigate_with_pid(self, target_x, target_y, target_yaw, timeout=60.0, max_speed=None, 
                         position_tolerance=None, angle_tolerance=None):
        """
        使用PID控制导航到目标点（直线路段）
        ...
        effective_pos_tol = position_tolerance if position_tolerance is not None else self.pid_position_tolerance
        effective_ang_tol = angle_tolerance if angle_tolerance is not None else self.pid_angle_tolerance
        
        rospy.loginfo(f"PID导航: 目标 ({target_x:.2f}, {target_y:.2f}, {target_yaw:.2f})")
        rospy.loginfo(f"参数: max_speed={effective_max_speed:.2f}, pos_tol={effective_pos_tol:.3f}, ang_tol={effective_ang_tol:.3f}")
        ...
        while not rospy.is_shutdown():
            ...
            # 判断是否到达目标位置
            if distance_error < effective_pos_tol:
                # 到达位置后，调整朝向
                final_angle_error = self.normalize_angle(target_yaw - current_yaw)
                if abs(final_angle_error) < effective_ang_tol:
    ...
    def navigate_with_teb(self, target_x, target_y, target_yaw, timeout=30.0):
        ...
        while retry_count < max_retries:
            ...
            if finished:
                ...
                if state == actionlib.GoalStatus.SUCCEEDED:
                    ...
                    rospy.logwarn(f"TEB导航失败，状态码: {state} (尝试 {retry_count + 1}/{max_retries})")
                    self.vibration_escape()
                    retry_count += 1
            else:
                rospy.logwarn(f"TEB导航超时 (尝试 {retry_count + 1}/{max_retries})")
                self.ac.cancel_goal()
                self.vibration_escape()
                retry_count += 1

        rospy.logerr("TEB导航在重试后仍然失败。")
        
        # 检查是否是停车点，并且距离足够近，才切换到PID
        # if self.is_parking_goal(target_x, target_y, target_yaw):
        ...
        rospy.logerr("TEB导航最终失败，且不满足切换到PID的条件。")
        return False

    def send_goal(self, x, y, yaw):
        """发送一个非阻塞的导航目标"""
        rospy.loginfo(f"发送非阻塞导航目标: ({x:.2f}, {y:.2f}, {yaw:.2f})")
    ...
    def escape_high_friction_surface(self, duration=1.0, vel_y=0.5, angular_z=0.8):
        ...
        """
        rospy.loginfo(f"执行脱困动作: vel_y={vel_y}, angulx yar_z={angular_z}, 持续{duration}秒")
